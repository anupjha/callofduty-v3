...
export const getServerSideProps: GetServerSideProps = withAuthGuard(async ({ req }) => {
  const cookies = cookie.parse(req.headers?.cookie || "");

  const { user, error } = await supabase.auth.api.getUser(cookies.access_token);
...
const AccountDetailsPage: React.FC<Props> = ({ user }) => {
  ...
  return (
    <div className="min-h-screen md:bg-slate-50">
      <Head>
        <title>Account Details | Call of duty</title>
      </Head>
      ...
      <main className="py-12">
        <div className="w-80 mx-auto md:w-96">
          <div className="flex justify-center mb-6">
            <Link href="/">
              <a>
                <Image src="/images/logo-with-text.png" alt="Call of duty logo" width={240} height={64} />
              </a>
            </Link>
          </div>
          <div className="md:rounded md:bg-white md:shadow-lg md:p-8">
            <h1 className="font-semibold text-center text-slate-600 mb-6">Fill up your account details</h1>
            <Formik
              initialValues={{
                fullname: fullname,
                username: username,
