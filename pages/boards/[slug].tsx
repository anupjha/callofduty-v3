...
export const getServerSideProps: GetServerSideProps = withAuthGuard(async () => {
  return {
    props: {},
  };
});
...
const BoardDetailPage: React.FC<Props> = ({}) => {
  ...
  const moveList = async ({ listId, toIndex }: { listId: number; toIndex: number }) => {
    if (!boardQuery.data) return;
  ...
  const handleMovement = (result: DropResult) => {
    ...
    if (result.type === "CARD") {
      ...
      const validToList = toListId !== fromListId && !!toListId ? Number(toListId) : undefined;
  ...
  return (
    <Layout>
      <Head>
        <title>{boardQuery.data && `${boardQuery.data.title} | `}Callofduty</title>
      </Head>

      <div className="bg-blue-600 h-screen flex flex-col">
        {deleteBoardMutation.status === "idle" && boardQuery.status === "success" && (
          <>
                  ...
                  <div className="flex items-start">
                    <DragDropContext onDragEnd={handleMovement}>
                      <Droppable droppableId="lists" direction="horizontal" type="LIST">
                        {(provided) => (
                          <div ref={provided.innerRef} className="flex items-start" {...provided.droppableProps}>
        ...
        {(deleteBoardMutation.status === "loading" || boardQuery.status === "loading") && (
          <div className="h-full w-full flex justify-center items-center">
            <Loading height={72} width={8} radius={16} margin={4} color="rgb(29 78 216)" />
          </div>
        )}
        {boardQuery.status === "error" && (
          <div className="h-full w-full flex flex-col items-center pt-32">
            <p className="font-semibold text-white text-3xl mb-6">Board not found</p>
            <p className="text-white text-sm text-center px-4 max-w-lg mb-8">
              This board may be private. If someone gave you this link, they may need to invite you to one of their
