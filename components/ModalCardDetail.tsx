...
import { MdClose, MdDelete, MdErrorOutline, MdOutlineCheckBox, MdTimer } from "react-icons/md";
...
const ModalCardDetail: React.FC<Props> = ({}) => {
  ...
  return (
    ...
    >
      <Head>
        <title>{cardQuery.data?.title && `${cardQuery.data.title} | `}Callofduty</title>
      </Head>
      ...
      <div className="w-full rounded bg-slate-100 p-5 pb-10 min-h-[480px] flex flex-col">
        {cardQuery.status === "success" && (
          <>
            ...
            <div className="flex items-start">
              ...
              <button className="text-slate-500 ml-12" type="button" onClick={close}>
                <MdClose size={24} />
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row">
              <div className="flex-1">
                {!!cardQuery.data.due_dates.length && <CardDueDate id={cardQuery.data.id} />}
                <CardDescription id={cardQuery.data.id} />
                {cardQuery.data.has_checklist && <CardChecklist id={cardQuery.data.id} />}
              </div>

              {/* Right Side Menu */}
              <div className="w-full mt-16 relative md:mt-0 md:w-48 md:ml-16">
                <div className="mb-8">
                  <p className="font-semibold text-xs text-slate-700 mb-3">Add to card</p>
                  <button
                ...
                <div>
                  <p className="font-semibold text-xs text-slate-700 mb-3">Actions</p>
                  <button
        ...
        {cardQuery.status === "loading" && (
          <div className="flex-1 flex justify-center items-center">
            <Loading height={36} width={4} radius={8} margin={2} color="rgb(203 213 225)" />
          </div>
        )}
        {cardQuery.status === "error" && (
          <div className="flex-1 flex flex-col justify-center items-center">
            <MdErrorOutline size={48} color="rgb(203 213 225)" />
            <p className="text-xs text-slate-700 mt-4">Failed to fetch card detail.</p>
          </div>
        )}
      </div>
    </Modal>
